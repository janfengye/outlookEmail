...
Web 登录页提供固定的登录有效期选项：7 天、30 天、90 天和 180 天，默认 30 天。有效期从登录成功时开始固定计算，后续访问页面或调用 API 不会续期；到期后页面、管理 API 和 SSE 会话均需重新登录。

登录页只在当前浏览器本地记忆上次选择的期限，不保存密码、Session Cookie 或绝对过期时间。服务端仍会校验期限选项，直接调用登录接口不能绕过固定选项和 180 天上限。浏览器扩展登录没有期限选择，默认建立 30 天会话。
